...
class MixStyle(nn.Cell):
    ...
    def __init__(self, p=0.5, alpha=0.1, eps=1e-6):
        super().__init__()
        self.p = p
        self.beta = np.random.beta(alpha, alpha)
        self.eps = eps
        self.alpha = alpha
    ...
    def construct(self, x):
        ...
        perm = ops.arange(batch_size - 1, -1, -1)  # inverse index
        perm_b, perm_a = perm.chunk(2)
        perm_b = perm_b[ops.randperm(batch_size // 2)]
        perm_a = perm_a[ops.randperm(batch_size // 2)]
        perm = ops.cat([perm_b, perm_a], 0)
